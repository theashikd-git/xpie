...
.sidebar { min-height: 100vh; background: #1f2d3d; }
.sidebar h4 { color: #fff; }
.sidebar a { color: #c2c7d0; padding: 12px 20px; display: block; font-size: 15px; text-decoration: none; }
.sidebar a:hover { background: #343a40; color: #fff; }
.card { border: none; border-radius: 10px; }
.card-header { background: #ffffff; font-weight: 600; border-bottom: 1px solid #e9ecef; }
.card-body { font-size: 14px; }
.item-row { display: flex; justify-content: space-between; margin-bottom: 6px; }
.small-text { font-size: 13px; color: #6c757d; }
.sidebar-logout { display: flex; align-items: center; padding: 12px 20px; color: #ff6b6b; font-weight: 600; text-decoration: none; border-top: 1px solid #343a40; transition: background 0.2s, color 0.2s; }
.sidebar-logout:hover { background: #343a40; color: #ff4c4c; }
</style>
</head>
<body>
...
<div class="row g-4">
  ...
  <div class="item-row">
    <div><strong><?= $row['product_name'] ?></strong><div class="small-text"><?= $row['department'] ?></div></div>
  ...
  <div class="item-row">
    <div><strong><?= $row['ip_address'] ?>/<?= $row['cidr'] ?></strong><div class="small-text">VLAN <?= $row['vlan_id'] ?> (<?= $row['vlan_name'] ?>)</div></div>
  </div>
  <?php } ?>
  </div>
  </div>
...
<div class="row g-4 mt-1">
  ...
  <div class="item-row">
    ...
    <div class="small-text"><?= date("d M Y", strtotime($row['created_at'])) ?></div>
  </div>
  <?php } ?>
  </div>
  </div>
  ...
  while($row=mysqli_fetch_assoc($warranty)){ 
    ...
    $remaining = ($interval->m > 0) ? $interval->m." month ".$interval->d." days" : $interval->d." days";
  ?>
  <div class="item-row">
    <strong><?= $row['product_name'] ?></strong>
...
<div class="row g-4 mt-1">
  ...
  <table class="table table-sm mb-0">
    <thead><tr><th>Product</th><th>Brand</th><th>Qty</th><th>Department</th></tr></thead>
    <tbody>
